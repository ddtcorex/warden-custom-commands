...
test_db_sync_download() {
    ...
    local backup_exists=$(docker exec --workdir / "${LOCAL_PHP}" bash -c "ls /var/www/html/var/${TEST_ENV_TYPE}-local_local-*.sql.gz 2>/dev/null | wc -l")
...
test_db_sync_upload() {
    ...
    if [[ "${output}" == *"local-to-dev"* ]]; then
        pass "DB sync upload - Used hyphenated 'local-to-dev' naming for transfer"
    else
        fail "DB sync upload" "Output did not contain expected 'local-to-dev' pattern. Output: ${output}"
    fi
}
test_db_sync_upload
