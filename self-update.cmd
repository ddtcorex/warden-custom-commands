...
## Apply patches for Warden (may already be applied, so ignore failures)
# Using -N (--forward) means it skips already-applied patches
if ! patch -N "${WARDEN_DIR}/commands/env.cmd" < "${WARDEN_HOME_DIR}/commands/patches/warden-fix-file-permissions.patch" 2>/dev/null; then
    info "Patch already applied or not needed"
fi
