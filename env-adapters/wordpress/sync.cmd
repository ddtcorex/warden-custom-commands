...
function sync_database() {
    ...
    if [[ "${SYNC_REMOTE_TO_REMOTE:-0}" -eq 1 ]]; then
        ...
        # Source DB info
        local src_db_config=$(${SSH_COMMAND} -p "${SOURCE_REMOTE_PORT}" "${SOURCE_REMOTE_USER}@${SOURCE_REMOTE_HOST}" "grep -E \"define\\s*\\(.*DB_(NAME|USER|PASSWORD|HOST)\" \"${SOURCE_REMOTE_DIR}/wp-config.php\"")
        ...
        # Destination DB info
        local dest_db_config=$(${SSH_COMMAND} -p "${DEST_REMOTE_PORT}" "${DEST_REMOTE_USER}@${DEST_REMOTE_HOST}" "grep -E \"define\\s*\\(.*DB_(NAME|USER|PASSWORD|HOST)\" \"${DEST_REMOTE_DIR}/wp-config.php\"")
...
if [[ "${SYNC_NO_FLUSH:-0}" -eq 0 && "${SYNC_DRY_RUN:-0}" -eq 0 ]]; then
    if [[ "${SYNC_REMOTE_TO_REMOTE:-0}" -eq 1 ]]; then
        if ${SSH_COMMAND} -p "${DEST_REMOTE_PORT}" "${DEST_REMOTE_USER}@${DEST_REMOTE_HOST}" "cd \"${DEST_REMOTE_DIR}\" && warden env exec php-fpm wp --info &>/dev/null"; then
            printf "🧹 \033[1;32mFlushing Cache via WP-CLI on %s ...\033[0m\n" "${SYNC_DESTINATION}"
            ${SSH_COMMAND} -p "${DEST_REMOTE_PORT}" "${DEST_REMOTE_USER}@${DEST_REMOTE_HOST}" "cd \"${DEST_REMOTE_DIR}\" && warden env exec -T php-fpm wp cache flush" || true
        fi
    else
