...
function sync_database() {
    ...
    if [[ "${DIRECTION}" == "upload" ]]; then
        ...
        if ! ${PV} -N "Importing" "${local_dump}" | ssh ${SSH_OPTS} -p "${ENV_SOURCE_PORT}" "${ENV_SOURCE_USER}@${ENV_SOURCE_HOST}" "${import_cmd}"; then
