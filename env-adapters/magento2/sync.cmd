...
function sync_database() {
    ...
    if [[ "${SYNC_REMOTE_TO_REMOTE:-0}" -eq 1 ]]; then
        ...
        # Source DB info
        local src_db_info=$(${SSH_COMMAND} -p "${SOURCE_REMOTE_PORT}" "${SOURCE_REMOTE_USER}@${SOURCE_REMOTE_HOST}" "php -r \"\\\$a=include \\\"${SOURCE_REMOTE_DIR}/app/etc/env.php\\\"; var_export(\\\$a['db']['connection']['default']);\"")
        ...
        # Destination DB info
        local dest_db_info=$(${SSH_COMMAND} -p "${DEST_REMOTE_PORT}" "${DEST_REMOTE_USER}@${DEST_REMOTE_HOST}" "php -r \"\\\$a=include \\\"${DEST_REMOTE_DIR}/app/etc/env.php\\\"; var_export(\\\$a['db']['connection']['default']);\"")
...
if [[ "${SYNC_NO_FLUSH:-0}" -eq 0 && "${SYNC_DRY_RUN:-0}" -eq 0 ]]; then
    ...
    if [[ "${SYNC_REMOTE_TO_REMOTE:-0}" -eq 1 ]]; then
        ${SSH_COMMAND} -p "${DEST_REMOTE_PORT}" "${DEST_REMOTE_USER}@${DEST_REMOTE_HOST}" "cd \"${DEST_REMOTE_DIR}\" && warden env exec -T php-fpm bin/magento cache:flush" || true
    else
